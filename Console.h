...
public:
    // 构造函数
    explicit Config(const QObject* parent = nullptr) {
        Q_UNUSED(parent);
    };
    // 析构函数
    ~Config() override{};
...
protected:
    std::vector<std::pair<QString,QString>> CommandList;
    std::vector<std::pair<QString, std::function<void()>>> CommandFuncList;
...
protected slots:
    void commandReceived(const QString& command) {
        ...
        if (CommandFuncList.size()) {
            for (auto& cmd:CommandFuncList) {
                if (cmd.first==command) {
                    cmd.second();
                    stopProcessing();
                    return;
...
public:
    // 构造函数
    explicit Console(QWidget* parent = nullptr,const Config& defaultConfig = Config(), const QSize& defaultSize = QSize());
    // 析构函数
    ~Console() override{
    }

    bool getProcessing() const {return isProcessing;}

    void setProcess(const bool& newValue) {
        this->isProcessing = newValue;
    }

    std::function<void()> startProcessing=[this]{this->isProcessing=true;};
    std::function<void()> stopProcessing=[this]{this->isProcessing=false;};

    void insertCommand(const QString& command,const QString& response) {
        this->CommandList.emplace_back(std::make_pair(command,response));
    }

    void insertCommand(const char* command,const char* response) {
        this->CommandList.emplace_back(std::make_pair(QString(command),QString(response)));
    }

    void insertCommand(const char* command,std::function<void()> response) {
        this->CommandFuncList.emplace_back(std::make_pair(QString(command),response));
    }
    ...
    ConsoleBufferStream operator<<(OutputT&& content) {
        ...
        else if constexpr(std::is_arithmetic_v<OutputT>) {
